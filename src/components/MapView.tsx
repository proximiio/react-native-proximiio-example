...
import Proximiio, { ProximiioContextProvider, ProximiioEvents, ProximiioLocation } from 'react-native-proximiio'
import ProximiioMapbox, { AmenitySource, FeatureCollection, GeoJSONSource, RoutingSource, UserLocationSource, Feature } from 'react-native-proximiio-mapbox'
...
export default class MapView extends Component<Props, State> {
  ...
  async onAction(action: MapControlAction) {
    ...
    if (action === 'center' && this.state.position) {
      this._camera?.flyTo([ this.state.position.lng, this.state.position.lat ], 300);
    }

    if (action === 'tracking') {
      console.log('toggle tracking', this.state.trackingEnabled)
  ...
  private onPress = (features: Feature[]) => {
    ...
    console.log('on press', features, 'points', points)
    if (points.length > 0) {
    ...
    return (
      <View style={{ flex: 1 }}>
        <MapboxGL.MapView
          ...
          <MapboxGL.Camera
            ref={(camera: MapboxGL.Camera) => {this._camera = camera}}
